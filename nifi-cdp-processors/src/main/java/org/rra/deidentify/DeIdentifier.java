...
public class DeIdentifier {
    ...
    /*public static final int[] X_INSTITUTION = {
            Tag.InstitutionAddress,
            Tag.InstitutionalDepartmentName,
            Tag.InstitutionalDepartmentTypeCodeSequence
    };*/

   /* public static final int[] X_DEVICE = {
            Tag.CassetteID,
            Tag.GantryID,
            Tag.GeneratorID,
    ...
    /*public static final int[] X_DATES = {
            Tag.CurveDate,
            Tag.CurveTime,
    ...
    /*public static final int[] Z_INSTITUTION = {
            Tag.InstitutionCodeSequence
    };*/

    /*public static final int[] Z_DATES = {
            Tag.AcquisitionDate,
            Tag.AcquisitionTime,
            Tag.AdmittingDate,
            Tag.AdmittingTime,
   ...
   /* public static final int[] Z_UID = {
    ...
    /*public static final int[] D_DEVICE = {
            Tag.DetectorID,
            Tag.DeviceSerialNumber,
            Tag.StationName,
    };*/

    /*public static final int[] D_INSTITUTION = {
            Tag.InstitutionName,
    };*/

    /*public static final int[] D_DATES = {
...
*/
    /*public static final int[] U_DEVICE = {
            Tag.DeviceUID,
    };
*/
    ...
    private final DeidentifyModel model;
    private int[] x;
    private int[] u;
    private final int[] o;

    public DeIdentifier(DeidentifyModel model, Option... options) {
        this.model = model;
        x = model.getX_Tags();
        u = model.getU_Tags();

        this.options = EnumSet.of(Option.BasicApplicationConfidentialityProfile, options);
        int[] z = model.getZ_Tags();
        int[] d = model.getD_Tags();
        if (!this.options.contains(Option.RetainDeviceIdentityOption)) {
            x = cat(x, model.getX_Devices());
            d = cat(d, model.getD_Devices());
            u = cat(u, model.getU_Devices());
        }
        if (!this.options.contains(Option.RetainInstitutionIdentityOption)) {
            x = cat(x, model.getX_Institutions());
            z = cat(z, model.getZ_Institutions());
            d = cat(d, model.getD_Institutions());
        }
        if (!this.options.contains(Option.RetainLongitudinalTemporalInformationFullDatesOption)) {
            x = cat(x, model.getX_Dates());
            z = cat(z, model.getZ_Dates());
            d = cat(d, model.getD_Dates());
        }
        if (!this.options.contains(Option.RetainUIDsOption)) {
            z = cat(z, model.getZ_UID());
        }
        o = cat(z, d);
        Arrays.sort(x);
        Arrays.sort(u);
    ...
    public DeidentifyModel getModel() {
        return this.model;
    }
    ...
    public void deidentify(Attributes attrs) {
        ...
        String acquisitionDateTime = attrs.getString(Tag.AcquisitionDateTime, null);
        deidentifyItem(attrs);
        correct(attrs);
        if (pid != null) {//OK both the same!
            // PID MUST BE Every time
            attrs.setString(Tag.PatientID, VR.LO, hash(pid));
        } else {
            attrs.setString(Tag.PatientID, VR.LO, hash(IDWithIssuer.pidOf(attrs)));
        }
        attrs.setString(Tag.PatientName, VR.PN, hash(IDWithIssuer.pidOf(attrs)));
        if (acquisitionDateTime != null) attrs.setString(Tag.AcquisitionDateTime, VR.DT, acquisitionDateTime);
